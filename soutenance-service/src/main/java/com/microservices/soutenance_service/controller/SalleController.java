package com.microservices.soutenance_service.controller;

import com.microservices.soutenance_service.dto.CreateSalleRequest;
import com.microservices.soutenance_service.dto.SalleResponse;
import com.microservices.soutenance_service.dto.UpdateSalleRequest;
import com.microservices.soutenance_service.service.SalleService;
import jakarta.validation.Valid;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import java.util.List;

@RestController
@RequestMapping("/api/salles")
public class SalleController {

    private final SalleService salleService;

    public SalleController(SalleService salleService) {
        this.salleService = salleService;
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public SalleResponse create(@Valid @RequestBody CreateSalleRequest request) {
        return salleService.create(request);
    }

    @PutMapping("/{id}")
    public SalleResponse update(@PathVariable Long id, @Valid @RequestBody UpdateSalleRequest request) {
        return salleService.update(id, request);
    }

    @GetMapping("/{id}")
    public SalleResponse getById(@PathVariable Long id) {
        return salleService.getById(id);
    }

    @GetMapping
    public List<SalleResponse> listAll() {
        return salleService.listAll();
    }

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void delete(@PathVariable Long id) {
        salleService.delete(id);
    }
}
